...
import { deleteCarousel, deleteQuestion } from "@/lib/api"
...
export const questionColumns: ColumnDef<Question>[] = [
    ...
    },
    {
        ...
        cell(props) {
            const queryClient = useQueryClient()
            const { mutate, isLoading } = useMutation({
                mutationKey: ["delete", "question", props.row.original.id],
                mutationFn: async () =>
                    await deleteQuestion(props.row.original.id!),
                onSuccess(data, variables, context) {
                    toast({
                        title: "Muvaffaqqiyatli amalga oshirildi!",
                    })
            ...
            return (
                <div className="flex gap-6">
                    <Link
                        className={cn(buttonVariants({ variant: "outline" }))}
                        href={`/edit-question/${props.row.original.id}`}
                    >
                        <svg
                            xmlns="http://www.w3.org/2000/svg"
                            x="0px"
